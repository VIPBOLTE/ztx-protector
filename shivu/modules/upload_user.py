...
from shivu import GROUP_ID, SUPPER_CHAT, OWNER_USERNAME, sudo_users
...
async def hupload(update: Update, context: CallbackContext) -> None:
    try:
        if update.effective_chat.type == "private":
            await update.message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("Join Support Chat", url=f"https://t.me/{SUPPER_CHAT}")]
                ])
            )
            return
        ...
        await update.message.reply_text(f'Character Upload Unsuccessful. Error: {str(e)}\nIf you think this is a source error, forward to: https://t.me/{SUPPER_CHAT}')
